...
const resolvers = {
    Query: {
        ...
        locations: (parent, args) => Locations,
        location: (parent, args) => Locations.find(location => location.id === args.id),
        participants: (parent, args) => Participants,
        participant: (parent, args) => Participants.find(participant => participant.id === args.id),
    },
    Event: {
        user: (parent, args) => {
            const user = Users.find(user => parseInt(user.id) === parent.user_id)
            return user
        },
        participant: (parent, args) => {
            const participant = Participants.find(participant => parseInt(participant.id) === parent.user_id)
            return participant
        },
        locations: (parent, args) => {
            const location = Locations.find(location => parseInt(location.id) === parent.location_id)
            return location
        }
    }
}
